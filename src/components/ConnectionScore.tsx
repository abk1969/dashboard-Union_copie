...
import { ConnectionService, MonthlyScore, ConnectionResult } from '../services/connectionService';

interface ConnectionScoreProps {
  className?: string;
}

const ConnectionScore: React.FC<ConnectionScoreProps> = ({ className = '' }) => {
  ...
  const connectionDays = score?.connection_days || 0;
  const currentStreak = score?.current_streak || 0;
