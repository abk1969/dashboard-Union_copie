...
console.log('➕ Script d\'ajout de nouvelles données à Supabase...\n');
...
function transformDataToSupabaseFormat(data) {
  ...
  return data.map((item, index) => {
    ...
    return {
      id: Date.now() + index, // ID unique basé sur timestamp + index
...
// Fonction pour ajouter les données par lots (sans supprimer les anciennes)
async function addDataInBatches(data, batchSize = 100) {
  console.log(`➕ Ajout des données par lots de ${batchSize}...`);
  ...
  for (let i = 0; i < totalRecords; i += batchSize) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.log(`✅ Lot ${batchNumber} ajouté avec succès`);
      } else {
  ...
  console.log(`\n📊 Résumé de l'ajout:`);
  console.log(`   ✅ Enregistrements ajoutés: ${addedCount}`);
  console.log(`   ❌ Erreurs: ${errorCount}`);
  console.log(`   📈 Taux de succès: ${((addedCount / totalRecords) * 100).toFixed(1)}%`);
}
...
async function addNewData() {
  try {
    ...
    console.log('\n⚠️  Prêt à ajouter les nouvelles données à Supabase');
    console.log(`📊 ${transformedData.length} enregistrements à ajouter`);
    console.log('🔄 Les anciennes données seront conservées');
    console.log('🚀 Démarrage de l\'ajout...\n');
    
    // Ajouter les données
    await addDataInBatches(transformedData);
